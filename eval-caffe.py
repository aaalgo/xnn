...
for it, path in shots:
    ...
    if os.path.exists(out):
        ...
        subprocess.check_call('%s model db --mode 1 --split %d --fold %d --annotate %s | tee %s' % (os.path.join(base_dir, 'xnn-roc'), args.split, args.fold, args.annotate, out), shell=True)
        os.remove('model/caffe.params')
    continue
    cc = []
